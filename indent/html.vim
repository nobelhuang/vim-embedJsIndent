...
" Source javascript indent
runtime indent/javascript.vim

let b:did_indent = 1

"setlocal indentexpr=HtmlIndent()
setlocal indentexpr=GetHtmlComIndent()
...
" By Nobel
"
" Indent combined for both HTML and JavaScript in HTML
function! GetHtmlComIndent()
    ...
    let js = '<script>\|<script\s\{1,\}type="text/javascript"'

    """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    " Originally written by Tye Zdrojewski <zdro@yahoo.com>, 05 Jun 2006
    "
    if 0 < searchpair(js, '', '</script>', 'nWb') && 0 < searchpair(js, '', '</script>', 'nW')
	" we're inside javascript
	if getline(lnum) !~ js && getline(v:lnum) !~ '</script>'
